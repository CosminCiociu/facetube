...
export default {
  ...
  getSingleVideoBySlug({ getters, commit }, { type, slug, showLoading = false }) {
    if ( ! getters.singleVideoBySlug({ type, slug }) ) {
      if (showLoading) {
        commit('SET_LOADING', true)
      }
      return fetchSingleVideo({ type, params: { slug } }).then(({ data: [ item ] }) => {
        commit('ADD_ITEM', { type, item  })
        if (showLoading) {
          commit('SET_LOADING', false)
        }
